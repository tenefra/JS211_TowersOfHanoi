...
First, lay out all the actions that need to take place in order for the game to function correctly.

We need:

1. A prompt to ask the question
1. A function that initiates the game actions and invokes other functions needed to make it work
1. A function to check if it's a valid input (This will be a conditional)
1. A function to check if it's a legal move (This will be a conditional)
1. A function that moves the piece from one array to another(this will only run if the other conditionals return true)
1. A function that checks if a win condition has been met
1. A function that restarts the game

For any invalid inputs or illegal moves we need to inform the user with console.log that the move was not valid or legal, and then we reset the prompt.
